...
function cchl_styles() {
  ...
  wp_register_style( 'mailchimp', '//cdn-images.mailchimp.com/embedcode/classic-081711.css' , array(), '0.1', 'screen');
	...
	wp_register_style( 'legacy', get_bloginfo('template_url') . '/css/legacy.44a51334.min.css', array(), CCHL_VERSION, 'screen' );

  wp_register_style( 'camara', get_bloginfo('template_url') . '/css/camara.1ffab4eb.min.css', array(), CCHL_VERSION, 'screen' );
...
global $post;
  ...
  if(WP_ENV != 'development') {
    wp_register_script( 'camara', get_bloginfo('template_url') . '/js/camara.4086899e.min.js', array('jquery'), CCHL_VERSION, true);
  } else {
